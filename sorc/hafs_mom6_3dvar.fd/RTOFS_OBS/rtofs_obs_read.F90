...
c     real      prf_lvl (n_depth, n_prf)
     ...
     6      form='unformatted')
      ...
      open(20,file='temp.txt',status='unknown')
      open(30,file='salt.txt',status='unknown')

      do i=1, n_in

!     HAT10
      if(prf_lat(i).ge.latmin.and.prf_lat(i).le.latmax.and.
     6   prf_lon(i).ge.lonmin.and.prf_lon(i).le.lonmax) then
...
!     profile
     ...
     6             prf_lvl(j,i)
      ...
      write(30,25) prf_dtg(i),prf_lat(i),prf_lon(i),
...
25    format(a12,1x,f6.3,1x,f8.3,3(1x,f6.3),1x,f6.1)
      close(20)
