...
if [[ "$DA_VAR_ENABLED" == "T" || "$DA_HOFX_ENABLED" == "T" ]]; then
    ...
    for ob_dir in $OBS_OUT_CTRL_DIR/*; do
        ...
        for f in $ob_dir/*_0000.nc; do
            ...
            # NOTE: we shouldn't need to concat anymore!
            ncks -h -g $obs_groups $ob_dir/${ob_var}.nc -A obsout/ctrl/$ob.$ymd.nc
        done
    done
...
if [[ "$DA_LETKF_ENABLED" == "T" ]]; then
    ...
    mkdir -p obsout/ens
    obs_groups="MetaData,ombg,oman,ObsValue,ObsError,EffectiveError0,EffectiveQC0"
    for ob_dir in $OBS_OUT_ENS_DIR/*; do
        ob=$(basename $ob_dir)
        echo "  Processing $ob"

        ncks -O -h -g $obs_groups $ob_dir/${ob}.nc obsout/ens/$ob.$ymd.nc &
    done
    wait
