...
# temporary hack until uv are added to envar
if [[ "$DA_MODE" == "letkf" ]]; then
    LETKF_STATE_VARS="Temp,Salt,ave_ssh,h,u,v"
else
    LETKF_STATE_VARS="Temp,Salt,ave_ssh,h"
fi
...
if [[ "$DA_ENABLED" == "T" ]]; then
    ...
    if [[ "$DA_MODE" != "letkf" && "$DA_SAVE_INC" =~ [tTyT1] ]]; then
        ...
        lof=`cd $INCR_TMP_DIR && ls *var.iter*.nc`
        for f in $lof; do
